...
use config::{
    ...
    FxIndexMap, CONFIG, FILE_EXT_PARQUET, SQL_FULL_TEXT_SEARCH_FIELDS,
};
use datafusion::{
...
async fn generate_index_inner(
    ...
    stream_name: &str,
    schema: SchemaRef,
    file_list_to_invalidate: Option<&Vec<FileKey>>,
    ...
    // We need this schema to create the record batches for the file list to invalidate
    ...
    let mut record_batches: Vec<arrow::record_batch::RecordBatch> = indexed_record_batches_to_merge
        .into_iter()
        .flatten()
        .collect();

    // Deregister the table from context, because we already have record batches.
    ctx.deregister_table("_tbl_raw_data")?;

    match caller_type {
        CallerType::Compactor => {
            let file_list_to_invalidate = file_list_to_invalidate.unwrap();
            ...
            let deleted: ArrayRef =
                Arc::new(BooleanArray::from(vec![true; len_of_columns_to_invalidate]));
            ...
            let (file_buf, file_meta) = record_batches.to_parquet_buf(original_file_size).await?;

            let index_filename = generate_index_file_name_from_compacted_file(
                &org_id,
                StreamType::Index,
                &stream_name,
                &new_file_key,
            );
            storage::put(&index_filename, bytes::Bytes::from(file_buf)).await?;
            ...
            // )
            // .await?;
            return Ok((index_filename, file_meta));
        }
        CallerType::Ingester => {
            ...
            .await?;
            Ok((arrow_file_name, FileMeta::default()))
        }
        _ => {
            return Err(anyhow::anyhow!("Invalid caller type".to_string(),));
        }
    }
}
...
pub(crate) async fn generate_index_on_ingester(
    ...
    .await
    .map(|(filename, _filemeta)| filename)
}

/// Generate an index on the compactor.
///
/// This function is used to generate an index file on the compactor. It also takes a list of
/// files to invalidate from the existing index which have been compacted into the new file.
...
//         len_of_columns_to_invalidate
//     ]));
//     let deleted: ArrayRef = Arc::new(BooleanArray::from(vec![true;
// len_of_columns_to_invalidate]));     let columns = vec![empty_terms, file_names, _timestamp,
// count, deleted];     let batch = RecordBatch::try_new(schema.into(), columns).unwrap();
